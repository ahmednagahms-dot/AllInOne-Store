    ...
    <div className="border border-gray-200 rounded-xl overflow-hidden text-xs bg-[#fbfbfc]">
      {rows.map(([key, value], i) => (
        <div
          key={key}
          className={cx(
            "grid grid-cols-2 border-b last:border-0 border-gray-100",
            i === 0 ? "bg-[#eaf3ff]" : i % 2 ? "bg-white" : "bg-[#f3f4f6]"
          )}
        >
          <span
            className={cx(
              "p-2 capitalize",
              i === 0 ? "text-[#2563eb] font-semibold" : "text-gray-600"
            )}
          >
            {String(key)}
          </span>
          <span
            className={cx(
              "p-2 break-words",
              i === 0 ? "text-[#2563eb] font-semibold" : "text-[#374151]"
            )}
          >
            {String(value)}
          </span>
...
function ShippingInfo() {
  return (
    <div className="rounded-xl border border-[#dce6ff] bg-[#f8faff] p-4 sm:p-5">
      ...
      <p className="text-base font-bold text-[#263653] mt-1">
        2 - 5 business days
      </p>
          ...
          <div key={step} className="relative z-10 min-w-0">
            <div
              className={cx(
                ...
                i < 3
                  ? "border-[#2149b8] bg-[#2149b8] text-white"
                  : "border-gray-300 bg-white text-gray-400"
              )}
            >
              {i < 3 ? <Star size={14} /> : <Truck size={13} />}
            </div>
            <p className="text-[10px] sm:text-[11px] font-semibold text-[#263653] mt-2 leading-4">
              {step}
            </p>
          </div>
        ))}
      ...
      <p className="mt-6 inline-flex items-center gap-2 text-xs font-semibold text-gray-500">
        <Truck size={14} className="text-[#2149b8]" /> Free shipping on orders
        over $50
      </p>
  ...
  const tabBtn = (key, label) => (
    <button
      ...
      className={cx(
        ...
        activeTab === key
          ? "text-[#2149b8] border-b-2 border-[#2149b8] pb-3 -mb-[14px]"
          : "text-gray-400 hover:text-[#2149b8]"
      )}
    >
      {label}
    </button>
    ...
    <div className="min-h-screen bg-[#f7f8fa] text-[#374151] px-1 sm:px-2 lg:px-3 py-4">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="text-[10px] text-gray-400 mb-4 px-1">
          ...
          <span className="mx-2">›</span>
          <span className="text-[#263653] font-medium">{product.name}</span>
        </div>
        ...
        <div className="bg-[#fbfbfc] border border-gray-200 rounded-2xl p-2.5 sm:p-3">
          <div className="grid lg:grid-cols-[0.98fr_1.02fr] items-start gap-4 lg:gap-5">
              ...
              <div className="flex gap-3">
                <div className="hidden sm:block w-[58px] shrink-0">
                  ...
                  >
                    {images.map((img, i) => (
                      <SwiperSlide key={i}>
                        <div className="w-full h-[68px] rounded-lg overflow-hidden border-2 border-transparent bg-[#fafafa] hover:border-[#2149b8] transition cursor-pointer">
                          <img
                            src={img.url}
                            alt=""
                ...
                <div className="relative flex-1 min-w-0">
                  {hasDiscount && (
                    <span className="absolute z-20 top-3 left-3 bg-[#2149b8] text-white text-[9px] px-2.5 py-1 rounded-md font-medium">
                      -
                  ...
                  >
                    {images.map((img, i) => (
                      <SwiperSlide key={i}>
                        <div className="h-[320px] sm:h-[390px] flex items-center justify-center bg-[#eef0f3]">
                          <img
                            src={img.url}
                            alt={product.name}
            ...
            <div className="pt-1 p-2 sm:p-3">
              ...
              <h1 className="text-2xl sm:text-3xl font-bold mt-1 text-[#374151]">
                {product.name}
              </h1>
              ...
              <div className="flex items-center gap-2 mt-3">
                ...
                <span
                  className={cx(
                    ...
                    stock > 0 ? "text-[#d28c16]" : "text-red-500"
                  )}
                >
              ...
              <div className="flex items-center gap-3 mt-5">
                {hasDiscount ? (
                  <>
                    <span className="text-2xl font-bold text-[#2149b8]">
              ...
              {stock > 0 && (
                <p className="text-[9px] mt-2 font-medium text-[#d28c16]">
              ...
              <div className="border-t border-gray-100 mt-5 pt-5">
                ...
                <div className="flex items-center border border-gray-200 rounded-lg w-fit overflow-hidden">
                  <button
                    ...
                    className="w-8 h-8 flex items-center justify-center rounded-md transition-all duration-200 hover:bg-[#eef3ff] hover:text-[#2149b8]"
                  >
                    <Minus size={11} />
                  </button>
                ...
                <div className="flex gap-2 mt-5">
                  <button
                    ...
                    disabled={stock <= 0}
                    className="flex-1 h-10 bg-[#2149b8] text-white rounded-lg text-[10px] font-semibold hover:bg-[#193b9d] hover:shadow-lg transition disabled:bg-gray-300 disabled:cursor-not-allowed"
                  >
                  ...
                  <button
                    ...
                    className={cx(
                      "w-10 h-10 border rounded-lg flex items-center justify-center transition-all duration-200",
                      isInWishlist(product._id)
                        ? "border-danger bg-danger/5 text-danger"
                        : "border-gray-300 text-gray-500 bg-white hover:border-gray-400 hover:text-danger hover:shadow-md"
                    )}
                  >
                    <Heart
                      size={15}
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-5 pt-1">
          {PAYMENT_FEATURES.map(([Icon, title, text]) => (
            <div
              key={title}
              className="group flex items-center gap-3 rounded-2xl border border-gray-200 bg-white p-4 transition-colors duration-300"
            >
        ...
        <section className="bg-white border border-gray-100 rounded-2xl mt-5 p-4 sm:p-5 lg:p-6">
          ...
          {activeTab === "description" && (
            <div className="mt-6 grid grid-cols-1 lg:grid-cols-[1.3fr_1fr] gap-5 lg:gap-6 items-start">
              <div>
                <h3 className="text-base sm:text-lg font-bold text-[#263653] mb-3">
                  {product.shortDescription || product.name}
                </h3>
                <p className="text-sm text-gray-500 leading-7 whitespace-pre-line">
                  {product.description ||
                    product.shortDescription ||
              ...
              <div className="space-y-5">
                <div>
                  <h3 className="text-base font-bold text-[#263653] mb-4">
                    Specifications
                  </h3>
              ...
              {isAuthenticated ? (
                ...
                >
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-primary-500 text-white font-medium rounded-lg hover:bg-primary-600 transition disabled:opacity-50"
                  >
                    {submittingReview ? "Submitting..." : "Submit Review"}
                  </button>
                </form>
              ) : (
                <div className="border border-gray-100 rounded-2xl p-5 text-sm text-gray-500">
                  <Link
                    to="/login"
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-6 pb-8">
            <h2 className="text-xl sm:text-2xl font-bold text-[#374151] mb-4">
              Related Products
            </h2>
